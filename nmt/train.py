...
def check_stats(stats, global_step, steps_per_stats, hparams, log_f):
    ...
    utils.print_out("  global step %d lr %g "
                    ...
                    (global_step, stats["learning_rate"], avg_step_time, speed,
                     train_ppl, avg_grad_norm, _get_best_results(hparams)),
                    log_f)
    ...
    while global_step < num_train_steps:
        ...
        if global_step - last_eval_step >= steps_per_eval:
            ...
            loaded_train_model.saver.save(
                train_sess,
                os.path.join(out_dir, 'checkpoints', "translate.ckpt"),
...
def _external_eval(model, global_step, sess, hparams, iterator,
    ...
    sess.run(iterator.initializer, feed_dict=iterator_feed_dict)
    output_dir = os.path.join(out_dir, "outputs_{}".format(label))
    os.makedirs(output_dir, exist_ok=True)
    output = os.path.join(out_dir, output_dir, "output_{}-{}".format(
        label, global_step))
